...
The DSH packages you launch above come from the sibling pnpm workspace at `../deepseek-harness` (`0.1.3-alpha.1`). `package.json` pins the same `0.1.3-alpha.1` versions; `node_modules` catches up via `npm install` once the registry publishes that line (until then it still holds the last published set). The browser half does not wait for the registry: `tsconfig.client.json` maps `@deepseek-ai/dsh-client-ui-primitives` to the sibling workspace source, so client typecheck always runs against the local harness tip. If the sibling moves ahead, run `npm install` to re-align.
