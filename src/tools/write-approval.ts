/**
 * The plan-review gate every write tool shows before Zotero is contacted.
 * The write plan rides the harness's user-questions seam with the
 * `plan-review` presentation intent, so a capable UI renders the plan itself
 * and the answer comes back as one of the offered labels. The gate fails
 * closed: no answerer, a subagent caller, or a malformed ask all refuse the
 * write — the model never writes past an unanswered plan. The user's Zotero
 * authorization dialog and its key remain the hard boundary; this is the
 * in-conversation confirmation layer on top.
